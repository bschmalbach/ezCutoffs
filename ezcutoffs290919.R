...
#generate data
data <- simulateData(model, model.type = "sem", sample.nobs = 1000)
#for multigroup cfa
...
ezCutoffs <- function(model, 
                      ...
                      alpha_level = 0.05,             # std_err = "none" !!! hatte es rausgenommen weil moreFitIndices es auch nicht gefunden hat
                      ...) {
  ...
  model_type <- lavInspect(fit, what="call")$model.type
  ...
  if (n_groups > 1) {
    ...
    data_generation <- function() {
      simulateData(pop_model, model.type = model_type, sample.nobs = group_sizes, group.label = group_labels)
    }
    ...
    data_generation <- function() {
      simulateData(pop_model, model.type = model_type, sample.nobs = n)
    }
    data_s_list <- vector("list", n_rep)
    data_s_list <- pbreplicate(n_rep, data_generation(), simplify = F)
  }
  ...
  simulation_stats <- data.frame(matrix(c(n_rep, n_conv, lavInspect(fit, what="call")$estimator, alpha_level),1,4))
...
a <- ezCutoffs(model=model, data=data, estimator = "MLM", n_rep = 100, which_fit=c("chisq","cfi","rmsea"), group="s2")
...
plot.ezCutoffs <- function(d) {
  ...
  for (i in 1:nrow(d[["summary"]])){
    ...
    hist_plots <- hist_plots + geom_vline(xintercept=d[["summary"]][i,1], color="blue", alpha=0.8)+
      annotate("text_repel", x=d[["summary"]][i,1], y = bar_height, angle=0, label="Empirical")+
