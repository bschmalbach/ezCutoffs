...
model <- "F1 =~ NA*a1 + a2 + a3 + 0.8*a4 + a5
          F2 =~ b1 + start(0.8)*b2 + b3 + equal('F2 =~ b2')*b4 + b5
          F1 ~~ 0*F2"

#generate data
data <- simulateData(model, sample.nobs = 1000)
#for multigroup cfa
...
a <- ezCutoffs(model=model, n_obs = 1000, estimator = "MLM", n_rep = 100, which_fit=c("chisq","cfi","rmsea"))
